...
RBuf_t* rbuf_create(int size) {
    ...
    if(size == 0)
        ...
        new_rb->size = size+1;
    new_rb->buf = (unsigned char *)malloc(new_rb->size);
    if(!new_rb->buf) {
        /* TODO - Error Messaeggs */
        free(new_rb);
        return NULL;
...
static void rbuf_update_size(RBuf_t *rb) {
    if(rb->wfx == rb->rfx)
        rb->used = 0;
...
int rbuf_read(RBuf_t *rb, unsigned char *out, int size) {
...
int rbuf_write(RBuf_t *rb, unsigned char *in, int size) {
...
int rbuf_find(RBuf_t *rb, unsigned char octet) {
    int i;
...
int rbuf_read_until(RBuf_t *rb, unsigned char octet, unsigned char *out, int maxsize)
{
    int i;
